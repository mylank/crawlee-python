...
def _get_from_cache_by_name(
    ...
    if issubclass(storage_class, Dataset):
        return _cache_dataset_by_name.get(name)  # type: ignore[arg-type]
    if issubclass(storage_class, KeyValueStore):
        return _cache_kvs_by_name.get(name)  # type: ignore[arg-type]
    if issubclass(storage_class, RequestQueue):
        return _cache_rq_by_name.get(name)  # type: ignore[arg-type]
...
def _get_from_cache_by_id(
    ...
    if issubclass(storage_class, Dataset):
        return _cache_dataset_by_id.get(id)  # type: ignore[arg-type]
    if issubclass(storage_class, KeyValueStore):
        return _cache_kvs_by_id.get(id)  # type: ignore[arg-type]
    if issubclass(storage_class, RequestQueue):
        return _cache_rq_by_id.get(id)  # type: ignore[arg-type]
